...
void CHistoryPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_LOGCHECK, m_logcheck);
}
...
void CHistoryPage::DrawDayDiagram(int day)
{
	...
	for (int i=0; i<dayrange; i++)
	{
		...
		else
		{
			...
			if (maxval == 0)
				maxval++;
			rect.top = rectClient.top + int( ( (float)rectClient.Height() * (sqrt(CUtil::int64_to_float(maxval)) - sqrt(CUtil::int64_to_float(temp)))) / sqrt(CUtil::int64_to_float(maxval)) );
		}
		if (day == today.GetDay())
		{
		...
		temp = m_pTheApp->m_wnd.m_ipStat.GetTotalReceived(today.GetDay());;
		if (!m_logcheck)
			rect.top = rectClient.top + int( ( (float)rectClient.Height() * (CUtil::int64_to_float(maxval) - CUtil::int64_to_float(temp))) / CUtil::int64_to_float(maxval) );
		else
		{
...
void CHistoryPage::DrawMonthDiagram(int month)
{
	...
	for (int i=0; i<12; i++)
	{
		...
		if (!m_logcheck)
			rect.top = int( ( (float)rectClient.Height() * (CUtil::int64_to_float(maxval) - CUtil::int64_to_float(temp))) / CUtil::int64_to_float(maxval) );
		else
		{
			if (temp == 0)
				temp++;
			...
			rect.top = int( ( (float)rectClient.Height() * (sqrt(CUtil::int64_to_float(maxval)) - sqrt(CUtil::int64_to_float(temp)))) / sqrt(CUtil::int64_to_float(maxval)) );
		}
		...
		temp = m_pTheApp->m_wnd.m_ipStat.GetTotalReceivedMonth(monthnum);
		if (!m_logcheck)
			rect.top = int( ( (float)rectClient.Height() * (CUtil::int64_to_float(maxval) - CUtil::int64_to_float(temp))) / CUtil::int64_to_float(maxval) );
		else
		{
